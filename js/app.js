 ...
 */
    ...
    (() => {
        ...
        const inputMasks = document.querySelector(".input__tel");
        if (inputMasks) {
        ...
        function initSliders() {
            if (document.querySelector(".main-slider__slider")) mainSwiper = new swiper_core_Swiper(".main-slider__slider", {
                modules: [ Navigation, EffectCreative, Mousewheel, Keyboard, Pagination, History ],
                direction: "vertical",
                observer: true,
                observeParents: true,
                slidesPerView: 1,
                ...
                on: {
                    ...
                    slideChange: function() {
                        menuSliderRemove();
                        menuLinks[mainSwiper.realIndex].classList.add("_active");
                    }
                }
            });
            if (document.querySelector(".slid__slider")) slidSwiper = new swiper_core_Swiper(".slid__slider", {
                modules: [ Navigation, EffectCreative, Pagination, Keyboard, Parallax ],
                observer: true,
                observeParents: true,
                slidesPerView: 1.04,
                spaceBetween: 0,
                ...
                on: {
                    init: function() {
                        let numbers = document.querySelector(".slid__slide.swiper-slide-active").querySelectorAll(".bottom-slid__number");
                        for (let i = 0; i < numbers.length; i++) {
                            const el = numbers[i];
                            outNum(el, Number(el.dataset.number));
                        }
                    },
                    slideChangeTransitionEnd: function() {
                        ...
                        if (document.querySelector(".slid__slide.swiper-slide-next")) {
                            let numbersNext = document.querySelector(".slid__slide.swiper-slide-next").querySelectorAll(".bottom-slid__number");
                            numbersNext.forEach((item => {
                                item.innerHTML = 0;
                            }));
                        }
                        if (document.querySelector(".slid__slide.swiper-slide-prev")) {
                            let numbersPrev = document.querySelector(".slid__slide.swiper-slide-prev").querySelectorAll(".bottom-slid__number");
                            numbersPrev.forEach((item => {
                                item.innerHTML = 0;
                            }));
                        }
                    }
            ...
            function outNum(el, num) {
                let startNum = 0;
                let time = 1e3;
                let step;
                if (!Number.isInteger(num)) step = .05; else if (num <= 100) step = 1; else if (num > 100 && num <= 1e3) step = 10; else if (num > 1e3 && num <= 1e4) step = 100; else if (num > 1e4) step = 1e3;
                let resultTime = Math.round(time / (num / step));
                let interval = setInterval((() => {
                    if (startNum + step > num) step /= 10;
                    startNum += step;
                    if (startNum === num || startNum.toFixed(2) == num) clearInterval(interval);
                    el.innerHTML = Number.isInteger(startNum) ? startNum : startNum.toFixed(2);
                }), resultTime);
            }
        }
        ...
        window.addEventListener("load", (() => {
            let preload = document.querySelector(".preload-0");
            let coverAll = document.querySelectorAll(".preload");
            let slider = document.querySelector(".main-slider__slider");
            coverAll = [ ...coverAll ];
            for (let i = 0; i < coverAll.length; i++) {
                ...
                if (i === coverAll.length - 1) setTimeout((() => {
